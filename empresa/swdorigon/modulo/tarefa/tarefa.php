...
class Tarefa extends Crud {
    ...
    function __construct() {
        ...
        if (!empty($_POST['listar'])) {
            if ($_POST['canceladas']) {
                $valor = $_POST['canceladas'];
            } else if ($_POST['concluidas']) {
                $valor = $_POST['concluidas'];
            } else {
                $valor = "< 4";
            }
            $this->sqlOrdem = sprintf($sqlTarefa, $valor);
        } else {
    ...
    public function iniciarCampos() {
        ...
        $this->setCampo("SIT_CODIGO[]", "Situação", 100, "combo", false, false);
